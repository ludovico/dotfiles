...
HTMLFILE="/tmp/${TIMESTAMP}-article.html"
PDFFILE="$HOME/Documents/newsboat/${TIMESTAMP}-article.pdf"

URL=$1
JSON=$(readable -s ~/.newsboat/readable-style.css --json "$URL")
...
<img\1src="\2"\3>/g' "$HTMLFILE"
sed -i '' -E 's#\$%7BformatId%7D#906#g' "$HTMLFILE"

# Fix srcset: Extract the highest-resolution image from src attribute
sed -i '' -E 's#src="([^"]* [0-9]+w, )*([^ ]+ [0-9]+w)"#src="\2"#g' "$HTMLFILE"
# Remove the resolution descriptor (e.g., "2000w") to keep only the URL
sed -i '' -E 's#src="([^ ]+) [0-9]+w"#src="\1"#g' "$HTMLFILE"
